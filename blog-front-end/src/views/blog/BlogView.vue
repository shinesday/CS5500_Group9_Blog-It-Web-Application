            ...
            <div class="me-view-comment-write">
              ...
              <el-row :gutter="20">
                <el-col :span="2" :offset="22">
                  <el-button type="text" @click="publishComment()">comment</el-button>
                </el-col>
              </el-row>
            </div>
    ...
    methods: {
      ...
      getArticle() {
        ...
        viewArticle(that.$route.params.id).then(data => {
          ...
          if (error !== 'error') {
            that.$message({type: 'error', message: 'article loading fail', showClose: true})
          }
        })
      },
      publishComment() {
      ...
      getCommentsByArticle() {
        ...
        getCommentsByArticle(that.article.id).then(data => {
          if(data.success){
             ...
             that.$message({type: 'error', message: 'comment loading fail', showClose: true})
          }
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: 'comment loading fail', showClose: true})
