    ...
    <el-container>
      ...
      <el-dialog title="abstract category tag"
        ...
        <el-form :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <el-form-item label="articles category" prop="category">
      ...
      return {
        ...
        rules: {
          summary: [
            ...
            {max: 80, message: '1111', trigger: 'blur'}
          ],
          category: [
            {required: true, message: '1111', trigger: 'change'}
          ],
          tags: [
            {type: 'array', required: true, message: '1111', trigger: 'change'}
          ]
        }
      }
    },
    ...
    methods: {
      getArticleById(id) {
        ...
        getArticleById(id).then(data => {
          ...
          if (error !== 'error') {
            that.$message({type: 'error', message: 'article loading failing....', showClose: true})
          }
        })
      },
      publishShow() {
        if (!this.articleForm.title) {
          this.$message({message: 'title cannot be empty', type: 'warning', showClose: true})
          return
        }

        if (this.articleForm.title.length > 30) {
          this.$message({message: 'title cannot longer then 30', type: 'warning', showClose: true})
          return
        }
        ...
        this.$refs[articleForm].validate((valid) => {
          if (valid) {
            ...
            let loading = this.$loading({
              lock: true,
              text: 'publish loading...'
            })

            publishArticle(article,this.$store.state.token).then((data) => {
              if(data.success){
                loading.close();
                that.$message({message: 'public success', type: 'success', showClose: true})
                that.$router.push({path: `/view/${data.data.id}`})
              }else{
                that.$message({message: error, type: 'public failing:'+data.msg, showClose: true});
              }
      ...
      cancel() {
        this.$confirm('cannot save cotinues?', 'hint', {
